...
export default function ConceptsManager() {
  ...
  function showEdit(concept) {
    setEditing(concept);
    setForm({ name: concept.name, category: concept.category, isActive: concept.isActive });
    setOpen(true);
  }
  ...
  return (
    <div className="animate-enter">
      <PageHeader eyebrow="Clasificación de gastos" title="Conceptos" description="Definí los conceptos que vas a seleccionar al registrar cada gasto, como Internet, alquiler o tarjeta de crédito." actions={<button className="btn-primary" onClick={showCreate}><CirclePlus size={17} /> Nuevo concepto</button>} />
        ...
        <div className="card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[620px] text-left">
              <thead className="border-b border-slate-200 bg-slate-50/80 text-xs uppercase tracking-wider text-slate-500"><tr><th className="px-6 py-4">Concepto</th><th className="px-6 py-4">Categoría</th><th className="px-6 py-4">Estado</th><th className="px-6 py-4 text-right">Acciones</th></tr></thead>
      ...
      <Modal open={open} onClose={() => !saving && setOpen(false)} title={editing ? "Editar concepto" : "Nuevo concepto"} description="El concepto y su categoría estarán disponibles al cargar un gasto.">
        <form onSubmit={save} className="space-y-4">
          ...
          <label className="block"><span className="mb-1.5 block text-sm font-bold text-slate-700">Categoría</span><select className="field" required value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}><option value="" disabled>Seleccionar...</option>{categories.filter((category) => category.isActive || category.name === form.category).map((category) => <option key={category.id} value={category.name}>{category.name}</option>)}</select></label>
          {editing && <label className="flex items-center justify-between rounded-xl bg-slate-50 p-3"><div><span className="block text-sm font-bold text-slate-700">Concepto activo</span><span className="text-xs text-slate-500">Estará disponible al cargar nuevos gastos</span></div><input type="checkbox" checked={form.isActive} onChange={(e) => setForm({ ...form, isActive: e.target.checked })} className="h-5 w-5 accent-violet-600" /></label>}
