...
const emptyForm = { name: "", category: "Servicios", defaultAmount: "", dueDay: 10, isActive: true };
...
export default function ConceptsManager() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const json = await readResponse(await fetch("/api/concepts", { cache: "no-store" }));
  ...
  function showEdit(concept) {
    setEditing(concept);
    setForm({ name: concept.name, category: concept.category, defaultAmount: concept.defaultAmount, dueDay: concept.dueDay, isActive: concept.isActive });
    setOpen(true);
  }
              ...
              <tbody className="divide-y divide-slate-100">
                {concepts.map((concept) => (
                  <tr key={concept.id} className={`transition hover:bg-slate-50/70 ${!concept.isActive ? "opacity-55" : ""}`}>
                    ...
                    <td className="px-6 py-4"><span className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600"><span className="h-2.5 w-2.5 rounded-full" style={{ background: CATEGORY_COLORS[concept.category] }} />{concept.category}</span></td>
      ...
      <Modal open={open} onClose={() => !saving && setOpen(false)} title={editing ? "Editar concepto" : "Nuevo concepto"} description="Estos valores se usarán como base al generar un mes.">
        <form onSubmit={save} className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label><span className="mb-1.5 block text-sm font-bold text-slate-700">Categoría</span><select className="field" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>{CATEGORIES.map((category) => <option key={category.value}>{category.value}</option>)}</select></label>
          ...
          <label className="block"><span className="mb-1.5 block text-sm font-bold text-slate-700">Importe habitual</span><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 font-bold text-slate-400">$</span><input className="field pl-7" type="number" min="0" step="0.01" required value={form.defaultAmount} onChange={(e) => setForm({ ...form, defaultAmount: e.target.value })} placeholder="0,00" /></div></label>
