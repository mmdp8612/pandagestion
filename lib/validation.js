export function parseConcept(input) {
  ...
  const closingDay = input.closingDay === "" || input.closingDay == null ? null : Number(input.closingDay);
...
export function parseExpense(input) {
  ...
  if (!category || category.length > 50) throw new Error("Seleccioná una categoría válida.");
  ...
  return { conceptId, category, month, amountCents: Math.round(amount * 100), dueDate, closingDate, notes };
}
